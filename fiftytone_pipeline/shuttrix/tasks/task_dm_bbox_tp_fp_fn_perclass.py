...
from shuttrix.operators.visualizer import TpFpFnPerClass, MultipleFiguresPlotter 


def init():
    parser = argparse.ArgumentParser(description="plot Tp/Fp/Fn in each dataset bbox.")
...
def run(ds_name: str,
    ...
    =======================================================================================================
                                    ...
                                    PLOT Tp/Fp/Fn PER BBOX IN {ds_name}
    ...
    visualizer = TpFpFnPerClass(
        op_name="TpFpFn PerClass",
        show=show,
        dm_eval_key=eval_key,
        title=f"TpFpFn PerClass for {ds_name}",
        step_value=step_value,
        step="BBoxEval",
    )
    visualizer.execute(dataset)
    
    # multiplot TpFpFn plots
